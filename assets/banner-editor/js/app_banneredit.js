...
require(["jquery", "sweetalert2"], function ($, Swal) {
  ...
  for (var i = 0; i < btns.length; i++) {
    btns[i].addEventListener("click", function () {
      ...
      data.append("link_imagen", $("#enlace1_" + this.id).val());
      ...
      var zoomid = zoomids[parseInt(this.id) - 1];
      data.append(
        "estado_hover",
        $("#checkbox" + zoomid.toString()).prop("checked") ? 1 : 0
      );
      if (set1.has(this.id)) {
      ...
      if (set3.has(this.id)) {
        ...
        data.append("link_boton", $("#enlace2_" + this.id).val());
      }
